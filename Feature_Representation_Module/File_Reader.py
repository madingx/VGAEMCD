...
def normalize_adjacency(adjacency):
    ...
    print('coo',coo_adjacency.shape)
    print('cnmmmm',sp.eye(coo_adjacency.shape[0]).shape)
    #wwwwww=np.delete(sp.eye(coo_adjacency.shape[0]), [i for i in range(coo_adjacency.shape[0])])
    wwwwww=sp.eye(coo_adjacency.shape[0])
    adjacency_ = coo_adjacency + wwwwww
...
def train_test_split(adjacency):
    ...
    #coo_adjacency_upper = coo_adjacency
    sp_adjacency = dense_to_sparse(coo_adjacency_upper)
    edges = sp_adjacency[0]
    print('边的个数',edges.shape[0])
    num_test = int(np.floor(edges.shape[0]/10000.))
    num_val = int(np.floor(edges.shape[0]/10000.))
